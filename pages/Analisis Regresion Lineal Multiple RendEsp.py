...
"""
st.set_page_config(page_title="Regresion Lineal Rendimiento", page_icon="img/dashboard.png", layout="wide")
...
colorCards= "#34495E"
colorPrev = "#74f210" 
st.subheader("Regresión Lineal de Rendmiento Esperado mensual con Machine Learning ")
...
df = pd.read_excel('datos/rend-datos.xlsx')
...
if option == 'BITCOIN':  
  ...
  #Regression coefficients (B0, B1, B2) / Coeficientes de regresión (B0, B1, B2)
  ...
  with st.expander("COEFICIENTE DE REGRESIÓN"):
    ...
    col2.metric('B1 COEFICIENTE:',value= f'{coefficients[0]:.4f}',delta=" Para X1 Nro. Ind. Merval (B1)")
    col3.metric('B2 COEFICIENTE',value= f'{coefficients[1]:.4f}',delta=" Para X2 Nro. de AL30 (B2):")
  ...
  with st.expander("MEDIDA DE VARIACIONES"):
    ...
    col3.metric('ERROR DE SUMA CUADRADA (SSE):',value= f'{sse:.4f}',delta="Squared(Y-Y_pred)")
    style_metric_cards(background_color=colorCards,border_left_color="#9900AD",border_color="#5B2C6F",box_shadow="#283747")
  
  # Print a table with predicted Y / Mostrar una tabla con la Y predicha
  with st.expander("TABLA DE PREDICCIÓN"):
    result_df = pd.DataFrame({'FECHA':df['FECHA'],'NRO DE RENDIMIENTO MERVAL':df['R-MERV'], 'NRO DE RENDIMIENTO AL30D': df['R-AL30D'], 'RENDIMIENTO BITCOIN | Actual Y': Y, 'Y_previsto': predictions})
    # Add SSE and SSR to the DataFrame / Agregar SSE y SSR al DataFrame
    result_df['SSE'] = sse
    result_df['SSR'] = ssr
    st.dataframe(result_df,use_container_width=True)  
    #download predicted excel / descargar el excel predicho 
  ...
  # Calculate residuals / Calcular residuos
    ...
    with col1:
      ...
      plt.xlabel('Actual Y | Rend.Esp Bitcoin')
  ...
  #with st.sidebar:
  with st.expander("DESEA AGREGAR DATOS PARA NUEVA PREVISIÓN DE RENDIMIENTO ESPERADO"):  
    st.subheader("Desea ingresar nuevo valor en Rend.Esp. para generar nueva previsión")
    with st.form("input_form",clear_on_submit=True):
      x1 = st.number_input("Digite Rend.Esp. Merval",)
      x2 = st.number_input("Digite Rend.Esp. AL30D",)
  ...
  if submit_button:
    ...
    predictions_formatted = [f"{value:.2f}" for value in new_prediction] #/ si se quiere mostrar con 2 decimales el resultado 
    with st.expander(f'("Ver Previsión de Rendimiento Esperado {option} %)"'):
      #st.write(f"<span style='font-size: 34px;color:green;'>Resultado Previsión de Rendimiento : </span> <span style='font-size: 35px;'> {new_prediction} % mensual </span>", unsafe_allow_html=True)
      # la opcion de arriba es para mostrar los resultados con mas de 2 decimales 
      st.write(f"<span style='font-size: 34px;color:{colorPrev};'>Resultado Previsión de Rendimiento : </span> <span style='font-size: 35px; color:{colorPrev};'> {', '.join(predictions_formatted)} % mensual </span>", unsafe_allow_html=True)
...
#st.write("---")
  ...
  # Print a table with predicted Y / Mostrar una tabla con la Y predicha
  with st.expander("TABLA DE PREDICCIÓN"):
    result_df = pd.DataFrame({'FECHA':df['FECHA'],'NRO DE RENDIMIENTO BITCOIN':df['R-BIT'], 'NRO DE RENDIMIENTO AL30D': df['R-AL30D'], 'RENDIMIENTO MERVAL | Actual Y': Y, 'Y_previsto': predictions})
    # Add SSE and SSR to the DataFrame / Agregar SSE y SSR al DataFrame
    result_df['SSE'] = sse
    result_df['SSR'] = ssr
    st.dataframe(result_df,use_container_width=True)  
  ...
  # Calculate residuals / Calcular residuos
    ...
    with col1:
      plt.scatter(Y, predictions)
      plt.plot([min(Y), max(Y)], [min(Y), max(Y)],color='red', label='Línea mejor ajuste')  # Best fit line / Línea de mejor ajuste
      plt.xlabel('Actual Y | Rend.Esp Merval')
      plt.ylabel('Previsto_Y')
      plt.grid(True)
      plt.legend()
      st.pyplot(plt)
  ...
  with st.expander("DESEA AGREGAR DATOS PARA NUEVA PREVISIÓN DE RENDIMIENTO ESPERADO"):  
    st.subheader("Desea ingresar nuevo valor en Rend.Esp. para generar nueva previsión")
    with st.form("input_form",clear_on_submit=True):
      x1 = st.number_input("Digite Rend.Esp. Bitcoin",)
      x2 = st.number_input("Digite Rend.Esp. AL30D",)
      submit_button = st.form_submit_button(label="Generar Previsión")
...
# -------------- desarrollo de analisis de rendimiento esperado sobre el BONO AL30D  --------------------
#st.write("---")
elif option == 'AL30D':  
  X = df[['R-BIT','R-MERV']]
  Y = df['R-AL30D']
  ...
  # Print a table with predicted Y / Mostrar una tabla con la Y predicha
  with st.expander("TABLA DE PREDICCIÓN"):
    result_df = pd.DataFrame({'FECHA':df['FECHA'],'NRO DE RENDIMIENTO BITCOIN':df['R-BIT'], 'NRO DE RENDIMIENTO MERVAL': df['R-MERV'], 'RENDIMIENTO AL30D | Actual Y': Y, 'Y_previsto': predictions})
    # Add SSE and SSR to the DataFrame / Agregar SSE y SSR al DataFrame
    result_df['SSE'] = sse
    result_df['SSR'] = ssr
    st.dataframe(result_df,use_container_width=True)  
  ...
  # Calculate residuals / Calcular residuos
    ...
    with col1:
      plt.scatter(Y, predictions)
      plt.plot([min(Y), max(Y)], [min(Y), max(Y)],color='red', label='Línea mejor ajuste')  # Best fit line / Línea de mejor ajuste
      plt.xlabel('Actual Y | Rend.Esp AL30D')
      plt.ylabel('Previsto_Y')
      plt.grid(True)
      plt.legend()
      st.pyplot(plt)
  ...
  with st.expander("DESEA AGREGAR DATOS PARA NUEVA PREVISIÓN DE RENDIMIENTO ESPERADO"):  
    ...
    with st.form("input_form",clear_on_submit=True):
      x1 = st.number_input("Digite Rend.Esp. Bitcoin",)
      x2 = st.number_input("Digite Rend.Esp. Merval",)
      submit_button = st.form_submit_button(label="Generar Previsión")
